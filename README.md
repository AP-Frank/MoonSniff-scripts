...
The main scripts the use needs to run are the [moonsniff-combined-series.sh](bash/moonsniff-combined-series.sh) and the [moonsniff-complete-test.sh](bash/moonsniff-complete-test.sh) scripts. The difference between both scripts is, that the first one measures once, does post-processing and repeats until all measurements are done.
...
### Naming Conventions
* Histograms need to follow the naming convention: ``<name>-<rate>.csv``, where ``<name>`` is the common part of all file names generated by MoonSniffs post-processing and ``<rate>`` is the rate which was measured.
